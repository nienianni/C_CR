...
void initial();//初始化数组
void generateNetwork_ini();//生成邻接矩阵，每个节点可朝哪些地方走
...
void generateNetwork_ini()
{
	...
	for (i = 0; i < NETWORK_SIZE+1; i++)
		for (j = 0; j < NETWORK_SIZE+1; j++)
			adjacentMatrix[i][j]=0 ;//初始化ER网络的邻接矩阵,全部置为0
	int count = 0;//用以统计网络中无向边的个数
	for (i = 0; i < NETWORK_SIZE+1; i++)
	{
		for (j = 0; j < NETWORK_SIZE+1; j++)
		{
			if ((i+1==j)||(i==0&&j!=0) || (j == 0 && i != 0)|| (i == NETWORK_SIZE&&j==1))
			{
...
void load_struct_people()
{
	int i, j, N;
	errno_t err;
	err = fopen_s(&fp1, "people_star2.txt", "r");
	if (err != 0)
	{
		puts("不能打开文件");
	}
...
void load_people_des()
{
	...
	for (i = 0; i < NETWORK_SIZE + 1; i++)
	{
		...
		for (j = 0; j < N; j++)
		{
			...
			{
				float r_where;
				r_where = (adjacentMatrix[city[i][j].sta][wSum_loc[i]])* (rand() / (RAND_MAX + 1.0));
				for (m = 0; m < NETWORK_SIZE + 1; m++)
				{
...
{   
	...
	{
		for (lambda = 0; lambda <= 0.0013; lambda = lambda + 0.000013)
		{
			load_I_pro_limit();
			pro=load_I_pro_zong();
			fprintf_s(fp2, "%f ", pro);
...
int load_stability_state()
{
	int i, j, N, S_c = 0, I_c = 0, stability;
	for (i = 0; i < NETWORK_SIZE + 1; i++)
	{
		if (i == 0)
			N = Nmax;
		...
		for (j = 0; j < N; j++)
		{
			if (city[i][j].ini_state == 'S'&&city[i][j].fin_state == 'I')//S 变为 I
				S_c++;
			if (city[i][j].ini_state == 'I'&&city[i][j].fin_state =='S')
				I_c++;
		}
	}
	stability = S_c - I_c;
